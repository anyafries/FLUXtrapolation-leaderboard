...
def get_pivot_df_with_scores(df, target, metric, aggfunc='median',
    ...
    has_val_strategy = 'val_strategy' in subset.columns
    index_cols = ['model', 'val_strategy'] if has_val_strategy else 'model'
...
def get_weighted_skill_scores(df, baseline_model='constant'):
    ...
    # Handle both flat and MultiIndex (model, val_strategy)
    if isinstance(df.index, pd.MultiIndex):
        # Find baseline: first row where first index level matches baseline_model
        matching = [idx for idx in df.index if idx[0] == baseline_model]
        if not matching:
            print(f"Warning: Baseline '{baseline_model}' not found.")
            return None, None
        baseline_errors = df.loc[matching[0]]
    else:
...
<head>
  ...
  <title>FLUXNET Leaderboard — {title}</title>
...
def create_html_leaderboard(
    ...
    # --- 3. Save ---
    if wrap_html:
        agg_label = aggfunc if isinstance(aggfunc, str) else 'custom'
        title = page_title or f"{target} ({agg_label} {metric.upper()})"
        heading = page_heading or f"{target} — {agg_label} {metric.upper()}"
        html_output = HTML_PAGE_TEMPLATE.format(
            title=title,
            heading=heading,
            table=table_html,
        )
